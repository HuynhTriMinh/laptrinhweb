...
class KhachHangController extends Controller
{
    ...
    public function getXoa($idKhachHang){
        ...
        $khach_hang_co_don_hang = DonHang::with('KhachHang')->where('idKhachHang', $idKhachHang)->count();

        if($khach_hang_co_don_hang == 0){
          $khachhang->delete();
          return redirect('admin/khachhang/danhsach')->with('thongbao', 'Xóa thành công');
        }
        else{
          return redirect('admin/khachhang/danhsach')->with('loi', 'Xóa không thành công. Bạn không thể xóa khách hàng có đơn hàng.');
        }
